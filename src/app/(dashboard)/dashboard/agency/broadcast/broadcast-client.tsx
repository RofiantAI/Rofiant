...
export function BroadcastClient({
  ...
  const t = useTranslations("dashboard.agency.broadcast");
  ...
  async function toggleAnnouncement(id: string, active: boolean) {
    const res = await fetch(`/api/agency/announcements/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteAnnouncement(id: string) {
    const res = await fetch(`/api/agency/announcements/${id}`, { method: "DELETE" });
    if (!res.ok) return;
  ...
  async function createScreen(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/agency/screens", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: screenTitle,
  ...
  async function toggleScreenPublished(id: string, published: boolean) {
    const res = await fetch(`/api/agency/screens/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteScreen(id: string) {
    const res = await fetch(`/api/agency/screens/${id}`, { method: "DELETE" });
    if (!res.ok) return;
                  ...
                  <DashboardCard key={s.id}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <code className="text-xs text-foreground-muted bg-background-tertiary px-1.5 py-0.5 rounded">
                            /dashboard/agency/screens/{s.slug}
                          </code>
                          {!s.published && (
                            <span className="text-[10px] uppercase tracking-wide text-foreground-muted">
                              {t("draft")}
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap">
                        <Link
                          href={`/dashboard/agency/screens/${s.slug}`}
                          target="_blank"
